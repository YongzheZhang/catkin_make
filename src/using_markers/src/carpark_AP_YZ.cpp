...
goal1.pose=carpark_parallel.step1_right.poses.at(indexP_lot);
   ...
   if(select_lot==0)
   {
          ...
          if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_left.poses.at(abs(indexV_lot-1)).position.x) <0.5
           &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_left.poses.at(abs(indexV_lot-1)).position.y)<0.5
           &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_left.poses.at(abs(indexV_lot-1)).orientation.w)<0.2
           &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_left.poses.at(abs(indexV_lot-1)).orientation.z)<0.2)
    {
       ...
       pose_pub.publish(goal2);
          ...
          if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_left.poses.at(abs(carpark_vertical.lot_center.poses.size()-1)).position.x) <0.5
           &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_left.poses.at(abs(carpark_vertical.lot_center.poses.size()-1)).position.y)<0.5
           &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_left.poses.at(abs(carpark_vertical.lot_center.poses.size()-1)).orientation.w)<0.2
           &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_left.poses.at(abs(carpark_vertical.lot_center.poses.size()-1)).orientation.z)<0.2)
     {
       ...
       if(indexV_lot!=carpark_vertical.lot_center.poses.size()-1)
       {
             ...
             if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_right.poses.at(indexV_lot+1).position.x) <0.5
              &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_right.poses.at(indexV_lot+1).position.y)<0.5
              &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_right.poses.at(indexV_lot+1).orientation.w)<0.2
              &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_right.poses.at(indexV_lot+1).orientation.z)<0.2)
       {
         ...
         // usleep(5000000);
          pose_pub.publish(goal2);
             ...
             if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_right.poses.at(0).position.x) <0.5
              &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_right.poses.at(0).position.y)<0.5
              &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_right.poses.at(0).orientation.w)<0.2
              &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_right.poses.at(0).orientation.z)<0.2)
        {
 ...
 //std::cout<<"step1_left.pose is "<<carpark_parallel.step1_left.poses.at(abs(indexP_lot-1))<<std::endl;
          ...
          if(fabs(msg->pose.pose.position.x-carpark_parallel.step1_left.poses.at(abs(indexP_lot-1)).position.x) <0.5
           &&fabs(msg->pose.pose.position.y- carpark_parallel.step1_left.poses.at(abs(indexP_lot-1)).position.y)<0.5
           &&fabs(msg->pose.pose.orientation.w- carpark_parallel.step1_left.poses.at(abs(indexP_lot-1)).orientation.w)<0.2
           &&fabs(msg->pose.pose.orientation.z- carpark_parallel.step1_left.poses.at(abs(indexP_lot-1)).orientation.z)<0.2)
      {
...
//ros::Publisher tree_pub = private_nh.advertise<visualization_msgs::Marker>("tree_decoration", 10);
    ...
    //plot car poses
     ...
     goalReach_sub=private_nh.subscribe("/toyota_odometry",10,step2Callback);
    ...
    carpark_vertical.line_array.color.r = 0.9;
    carpark_vertical.line_array.color.g = 0.9;
    carpark_vertical.line_array.color.a = 0.7;
...
//    carpark_vertical.line_array.color.a = 1.0;
    ...
    for(int i=0;i<carpark_vertical.number*1+1;i++){
     ...
     if(i<carpark_vertical.number){
        ...
        lot.position.x = pt.x * cos(carpark_vertical.yaw) - pt.y * sin(carpark_vertical.yaw);
        lot.position.y = pt.x * sin(carpark_vertical.yaw) + pt.y * cos(carpark_vertical.yaw);
        tf::quaternionTFToMsg(tf::createQuaternionFromYaw(carpark_vertical.yaw-1*PI/4),lot.orientation);
        ...
        lot.position.x = pt.x * cos(carpark_vertical.yaw) - pt.y * sin(carpark_vertical.yaw);
        lot.position.y = pt.x * sin(carpark_vertical.yaw) + pt.y * cos(carpark_vertical.yaw);
        tf::quaternionTFToMsg(tf::createQuaternionFromYaw(carpark_vertical.yaw-6*PI/8),lot.orientation);
    ...
    carpark_parallel.line_array.color.a = 0.7;
