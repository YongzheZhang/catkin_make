...
ros::Publisher pose_pub,car_polygon_pub,car_polygon_left_pub,car_polygon_vector_pub ,half_circle_poses_pub,half_circle_poses_2_pub, half_circle_pub, half_circle_left_pub;
...
//class for checking points if lie in a square
...
int indexV_points, indexV_lot, indexP_points, indexP_lot,indexdegree_points, indexdegree_lot, select_lot;


void clickedptCallback(const geometry_msgs::PointStamped::ConstPtr& msg)
{
    double dist_to_lotcenter_v= 10.0;
    double dist_to_lotcenter_p =10.0;
    double dist_to_lotcenter_d =10.0;

   poses_car.header.frame_id=poses_car_2.header.frame_id="/map";
    ...
    nav_msgs::Path car_poses,car_poses_left;

    goal1.header.frame_id
    =car_polygon.header.frame_id
    =car_polygon_2.header.frame_id
    =car_poses.header.frame_id
   ...
   // std::cout<<"car orientation "<<carpark_parallel.direction.pose.pose.orientation<<std::endl;
    ...
    for (int i=0; i<carpark_vertical.lot_center.poses.size();i++){
        ...
        if(temp<dist_to_lotcenter_v){
            dist_to_lotcenter_v=temp;
            indexV_points=2*i;
            indexV_lot=i;
            select_lot=0;
        }
    ...
    for (int i=0; i<carpark_parallel.lot_center.poses.size();i++){
        ...
        if(temp<dist_to_lotcenter_p){
            dist_to_lotcenter_p=temp;
            indexP_points=2*i;
            indexP_lot=i;
            select_lot=1;
        }
    ...
    for (int i=0; i<carpark_degree.lot_center.poses.size();i++){
        ...
        if(temp<dist_to_lotcenter_d){
            dist_to_lotcenter_d=temp;
    ...
    if(select_lot==0)
    {
        ...
        if(fabs(area-Vparking_lot_area)<0.5)
        {
            ...
            //..........................Project out poses for checking collision---------------------------
            int l=0;
            //One side-plot
            for(int t=0;t<=circle_num;t++)
            {
                ...
                points.x=carpark_vertical.lot_reference.poses.at(indexV_lot).position.x-(radius_min+t*distance)*cos(carpark_vertical.yaw);
                points.y=carpark_vertical.lot_reference.poses.at(indexV_lot).position.y-(radius_min+t*distance)*sin(carpark_vertical.yaw);
    ...
    if(select_lot==1)
    {
        ...
        if(fabs(area-Pparking_lot_area)<0.5)
        {


            if(carpark_parallel.direction.pose.pose.orientation.w>=0.0 && carpark_parallel.direction.pose.pose.orientation.w<=0.72)
            {
                if(carpark_parallel.lot_center.poses.at(0).orientation.z>=0 && carpark_parallel.lot_center.poses.at(0).orientation.z<=1)
                {



                    goal1.pose=carpark_parallel.step1_left.poses.at(indexP_lot);
                    pose_pub.publish(goal1);
                ...
                else
                {
                    ...
                    goal1.pose=carpark_parallel.step1_right.poses.at(indexP_lot);
                    pose_pub.publish(goal1);
                }
            }
            ...
            else if(carpark_parallel.direction.pose.pose.orientation.w>0.7 && carpark_parallel.direction.pose.pose.orientation.w<=1)
            {
                if(carpark_parallel.lot_center.poses.at(0).orientation.z>=0 && carpark_parallel.lot_center.poses.at(0).orientation.z<=1)
                {
                    goal1.pose=carpark_parallel.step1_right.poses.at(indexP_lot);
...
void step2Callback(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_left.poses.at(abs(indexV_lot)).position.x) <0.1
            &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_left.poses.at(abs(indexV_lot)).position.y)<0.1
            &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_left.poses.at(abs(indexV_lot)).orientation.w)<0.05
            &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_left.poses.at(abs(indexV_lot)).orientation.z)<0.05)
    {

        std::cout<<"step1_left.pose is "<<carpark_vertical.step1_left.poses.at(abs(indexV_lot))<<std::endl;

        goal2.pose=carpark_vertical.lot_reference.poses.at(indexV_lot);
    ...
    else if(fabs(msg->pose.pose.position.x-carpark_vertical.step1_right.poses.at(indexV_lot).position.x) <0.1
            &&fabs(msg->pose.pose.position.y- carpark_vertical.step1_right.poses.at(indexV_lot).position.y)<0.1
            &&fabs(msg->pose.pose.orientation.w- carpark_vertical.step1_right.poses.at(indexV_lot).orientation.w)<0.05
            &&fabs(msg->pose.pose.orientation.z- carpark_vertical.step1_right.poses.at(indexV_lot).orientation.z)<0.05)
    {

        std::cout<<"step1_right.pose is "<<carpark_vertical.step1_right.poses.at(indexV_lot)<<std::endl;
    ...
    else if(fabs(msg->pose.pose.position.x-carpark_parallel.step1_right.poses.at(indexP_lot).position.x) <0.1
            &&fabs(msg->pose.pose.position.y- carpark_parallel.step1_right.poses.at(indexP_lot).position.y)<0.1
            &&fabs(msg->pose.pose.orientation.w- carpark_parallel.step1_right.poses.at(indexP_lot).orientation.w)<0.05
            &&fabs(msg->pose.pose.orientation.z- carpark_parallel.step1_right.poses.at(indexP_lot).orientation.z)<0.05)
    {

        std::cout<<"step1_right.pose is "<<carpark_parallel.step1_right.poses.at(indexP_lot)<<std::endl;

        goal2.pose=carpark_parallel.lot_reference_2.poses.at(indexP_lot);
        ros::Duration(3.0).sleep();
        pose_pub.publish(goal2);
    }

    //std::cout<<"step1_right.pose is "<<carpark_parallel.step1_right.poses.at(abs(indexP_lot-1))<<std::endl;

    else if(fabs(msg->pose.pose.position.x-carpark_parallel.step1_left.poses.at(indexP_lot).position.x) <0.1
            &&fabs(msg->pose.pose.position.y- carpark_parallel.step1_left.poses.at(indexP_lot).position.y)<0.1
            &&fabs(msg->pose.pose.orientation.w- carpark_parallel.step1_left.poses.at(indexP_lot).orientation.w)<0.05
            &&fabs(msg->pose.pose.orientation.z- carpark_parallel.step1_left.poses.at(indexP_lot).orientation.z)<0.05)
    {

        goal2.pose=carpark_parallel.lot_reference.poses.at(indexP_lot);
        ros::Duration(3.0).sleep();
        pose_pub.publish(goal2);
    }
}
...
int main( int argc, char** argv )
{
    ...
    ros::Subscriber sub = nh.subscribe("clicked_point", 10, clickedptCallback);
    //ros::Publisher tree_pub = private_nh.advertise<visualization_msgs::Marker>("tree_decoration", 10);
    ros::Publisher carpark_vertical_pt_pub = nh.advertise<visualization_msgs::Marker>("carpark_vertical_pt", 10);
    ros::Publisher carpark_vertical_li_pub = nh.advertise<visualization_msgs::Marker>("carpark_vertical_li", 10);
    ros::Publisher carpark_parallel_pt_pub =nh.advertise<visualization_msgs::Marker>("carpark_parallel_pt", 10);
    ros::Publisher carpark_parallel_li_pub =nh.advertise<visualization_msgs::Marker>("carpark_parallel_li", 10);
    ros::Publisher carpark_degree_pt_pub =nh.advertise<visualization_msgs::Marker>("carpark_degree_pt", 10);
    ros::Publisher carpark_degree_li_pub =nh.advertise<visualization_msgs::Marker>("carpark_degree_li", 10);
    ...
    //    carpark_vertical.line_array.color.g = 1.0;
    //    carpark_vertical.line_array.color.b = 1.0;
    //    carpark_vertical.line_array.color.a = 1.0;
    ...
    for(int i=0;i<carpark_vertical.number*1+1;i++){
        ...
        if(i<carpark_vertical.number){
            ...
            pt.x = carpark_vertical.origin.position.x + 3*carpark_vertical.width/2-0.1+ i*carpark_vertical.width;
            pt.y = carpark_vertical.origin.position.y - carpark_vertical.depth/2-0.20;
            lot.position.x = pt.x * cos(carpark_vertical.yaw) - pt.y * sin(carpark_vertical.yaw);
            lot.position.y = pt.x * sin(carpark_vertical.yaw) + pt.y * cos(carpark_vertical.yaw);
            ...
            pt.x = carpark_vertical.origin.position.x - carpark_vertical.width/2+0.1 + i*carpark_vertical.width;
            pt.y = carpark_vertical.origin.position.y - carpark_vertical.depth/2-0.20;
            lot.position.x = pt.x * cos(carpark_vertical.yaw) - pt.y * sin(carpark_vertical.yaw);
            lot.position.y = pt.x * sin(carpark_vertical.yaw) + pt.y * cos(carpark_vertical.yaw);
    ...
    carpark_parallel.line_array.color.r = 0.9;
    carpark_parallel.line_array.color.b = 0.9;
    carpark_parallel.line_array.color.a = 0.8;
    ...
    carpark_parallel.point_array.header.frame_id
            ...
            =carpark_parallel.lot_reference_2.header.frame_id
    ...
    for(int i=0;i<carpark_parallel.number*1+1;i++){
        ...
        if(i<carpark_parallel.number){
            ...
            pt.x = carpark_parallel.origin.position.x +0.8+carpark_parallel.width + i*carpark_parallel.width;
            ...
            pt.x = carpark_parallel.origin.position.x -0.8 + i*carpark_parallel.width;
            pt.y = carpark_parallel.origin.position.y -1.7;
            lot.position.x = pt.x * cos(carpark_parallel.yaw) - pt.y * sin(carpark_parallel.yaw);
            lot.position.y = pt.x * sin(carpark_parallel.yaw) + pt.y * cos(carpark_parallel.yaw);
    ...
    carpark_degree.line_array.color.g = 0.5;
    carpark_degree.line_array.color.b = 0.0;
